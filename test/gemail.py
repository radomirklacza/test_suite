...
# TODO combine below functions - there is no need to double the code
...
# this function fetch emails from gmail and look for reset password link
def fetch_reset_link(username, password, mailalias = None):
    ...
    result, data = mail.search(None,'(TO "%s" SUBJECT "Password reset")' % (mailalias))
    ids = data[0] # data is a list.
    id_list = ids.split() # ids is a space separated string
    latest_email_id = id_list[-1] # get the latest
