...
def simpletest(fakeuser=None, piuser=None):
    ...
    test_status.append(user_create)
    if test_status[0][0] == True :
        test.user.logout(driver)
    ...
    test.piuser.upgrade_user_to_pi(driver, config.portal['url'], piuser, fakeuser, testname, errordb, datadb, concurrent_users, display=dis)
    test.user.logout(driver)
    test.user.signin(driver, fakeuser['email'], fakeuser['password'], config.portal['url'], testname, errordb, datadb, concurrent_users, display=dis)
    test.user.logout(driver)
    test.user.is_pi(driver, config.portal['url'], fakeuser, testname, errordb, datadb, concurrent_users, display=dis)
    test.user.logout(driver)
    test.piuser.downgrade_user_from_pi(driver, config.portal['url'], piuser, fakeuser, testname, errordb, datadb, concurrent_users, display=dis)
    test.user.logout(driver)
    if test.user.is_pi(driver, config.portal['url'], fakeuser, testname, errordb, datadb, concurrent_users, display=dis):
    ...
    #
    test.user.signin(driver, fakeuser['email'], fakeuser['password'], config.portal['url'], testname, errordb, datadb, concurrent_users, display=dis)
    test.project.create(driver, config.portal['url'], fakeuser['project'], testname, errordb, datadb, concurrent_users, display=dis)
    test.user.logout(driver)
    test.piuser.validate_project(driver, config.portal['url'], piuser, fakeuser, testname, errordb, datadb, concurrent_users, display=dis)
    test.user.logout(driver)
    test.piuser.delete_project(driver, config.portal['url'], piuser, fakeuser, testname, errordb, datadb, concurrent_users, display=dis)
    test.user.logout(driver)
    # # clean up
    #
    test.piuser.delete_user(driver, config.portal['url'], piuser, fakeuser, testname, errordb, datadb, concurrent_users, display=dis)
    test.user.logout(driver)
